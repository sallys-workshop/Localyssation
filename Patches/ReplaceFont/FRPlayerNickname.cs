    ...
    internal static class FRPlayerNickname
    {
        ...
        public static void Player_Handle_ClientParameter_Postfix(Player __instance)
        {
            if (__instance._nicknameTextMesh.enabled)
                FRUtil.replaceTmpFont(__instance._nicknameTextMesh, Localyssation.currentLanguage.info.fontReplacementLibrationSans);
            if (__instance._globalNicknameTextMesh.enabled)
                FRUtil.replaceTmpFont(__instance._globalNicknameTextMesh, Localyssation.currentLanguage.info.fontReplacementLibrationSans);
        }
    }
}
